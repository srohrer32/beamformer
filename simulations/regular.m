...
for phase_shift = 0: (pi/32) : ((31*pi)/32) 	
	...
	z_1 = sin(omega*sqrt((x+4).^2+(y).^2)+phase_shift);
	z_2 = sin(omega*sqrt((x+2).^2+(y).^2)+phase_shift);
	z_3 = sin(omega*sqrt((x).^2+(y).^2)+phase_shift);
	z_4 = sin(omega*sqrt((x-2).^2+(y).^2)+phase_shift);
